# troubleshooting-linux-aws-project
This project is all about breaking things and learning how to fix them. I simulated real server issues like 502 errors, config failures, permission problems, and disk issues, then debugged them step by step. 

## Overview

This project demonstrates how to set up and manage a real-world Linux server on AWS.

Instead of just deploying a server, this project focuses on **learning through real scenarios** by:
- Deploying a backend application  
- Configuring a reverse proxy using Nginx  
- Intentionally creating real-world issues (as I said, still less than the glasses that I have broken)
- Debugging and resolving them step by step

I have tried writing the entire project in a **beginner-friendly, teaching style**, so anyone can follow along easily.

---

## Learnings

By going through this project, I understood:

- How a real server architecture works (Nginx + Backend)
- How to debug common production issues
- How to read error messages and troubleshoot logically
- Core Linux troubleshooting skills used in real jobs

---
